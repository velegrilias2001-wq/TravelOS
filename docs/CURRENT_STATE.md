...
- Current development branch: `feature/import-pdf-calendar-v1`
- Branch point: `5273fa6` — feat: add Import calendar ZIP V1
- Import PDF-embedded iCalendar V1 is implemented on this branch: a chosen PDF can yield a calendar only when it actually contains a `BEGIN:VCALENDAR` block, including FlateDecode streams. Confirmation PDFs without a calendar fail closed. There is no AI parsing of invoice or ticket prose. Automated tests exist. No native device rehearsal was run for the PDF extractor.
...
- Home, Trips, and Bookings can open `/import` to choose an `.ics` file, a zip of calendars, a PDF that embeds a calendar, or paste iCalendar text. Extracted events enter a review queue and do not become bookings until accepted onto an existing trip.
