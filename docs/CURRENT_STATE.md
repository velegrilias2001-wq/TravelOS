...
- Current development branch: `feature/import-email-calendar-v1`
- Branch point: `c9e1aa2` — feat: add Import ICS file picker V1
- Import email-wrapped iCalendar V1 is implemented on this branch: paste or a chosen file may be a raw `.ics`, UTF-16 calendar bytes, or an email that contains a `text/calendar` part. Extraction feeds the same review queue. PDF/HTML/AI extraction is still out. Automated tests exist. An Android Pixel 8 rehearsal on 2026-09-02 chose `e2e-email.eml` from Downloads and opened review as HIGH CONFIDENCE · PENDING without writing a booking.
...
- Migration version 11 adds `import_batches` and `import_claims`. Import accepts iCalendar (`.ics`) text from paste or a chosen local file. An email that contains a `text/calendar` / `application/ics` part, or a visible `BEGIN:VCALENDAR` block, is extracted first. UTF-16 BOM calendars are decoded. There is no PDF/ZIP/image extraction and no AI parsing. Files larger than 512 KiB fail closed. Non-calendar files fail closed. iCloud entitlements were not enabled.
...
- Re-pasting or re-choosing the same calendar is idempotent by content hash of the extracted iCalendar, not the email wrapper. A chosen file uses its basename as `sourceLabel`; paste uses `Pasted calendar` or `Pasted email`.

Automated-test evidence: `tests/import-ics.test.cjs`, `tests/import-review.test.cjs`, `tests/import-review-migration.test.cjs`, `tests/import-calendar-file.test.cjs`, and `tests/import-calendar-extract.test.cjs`. Android Pixel 8 development-build evidence for paste/review/accept/delete, choosing a local `.ics` file, and extracting a `text/calendar` email on 2026-09-02 is recorded under Testing and release readiness.

Current limitations: PDF/ZIP/Office/image extraction, itinerary-stop claims, and AI-assisted extraction are still out of scope. iOS has not been rebuilt with `expo-document-picker`. Accepted bookings still require the traveler to confirm reservation details in Bookings.
...
- Broader import formats. Native import now extracts iCalendar from paste, a chosen file, UTF-16 calendar bytes, or an email `text/calendar` part. PDF/Office/image extraction and AI parsing remain out. Android Pixel 8 rehearsed choosing a local `.ics` file and an `.eml` with a calendar part; iOS was not rebuilt.
