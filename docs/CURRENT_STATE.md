...
The latest local git work is on `docs/expo-sqlite-rehearsal-7-15` through Phase 6 local restore, credentials/versioning, EAS project link, and the first Android preview cloud build (`ca98eafd-e956-4e05-9a62-78aeddfdfa94`). Live Android `user_version` remains 17. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); this branch still has no upstream, so GitHub Actions has not run for it. EAS project `@velegris/TravelOS` is linked with a remote Android keystore and a finished preview APK. Play tracks and Maps package/SHA verification remain open. iOS remains last.
