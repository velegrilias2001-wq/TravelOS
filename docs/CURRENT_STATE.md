...
- Current development branch: `feature/import-office-calendar-v1`
- Branch point: `80ebaf4` — feat: add Import PDF-embedded iCalendar V1
- Import Office-embedded iCalendar V1 is implemented on this branch: Word/Excel/PowerPoint Open XML packages yield a calendar only when visible text contains a `BEGIN:VCALENDAR` block, including text split across Office runs. Images fail closed. There is no OCR and no AI parsing of confirmation prose. Automated tests exist. No native device rehearsal was run for the Office extractor.
...
- Home, Trips, and Bookings can open `/import` to choose an `.ics` file, a zip of calendars, a PDF or Office file that embeds a calendar, or paste iCalendar text. Extracted events enter a review queue and do not become bookings until accepted onto an existing trip.
...
Import Office-embedded iCalendar V1 verification on 2026-09-02 re-ran `npx tsc --noEmit` and `npm test` (217 application tests passing, including the calendar-office suite). `git diff --check` was clean. No native rebuild was required. No device rehearsal was run for the Office extractor. Images fail closed without OCR.
...
- Broader import formats. Native import now extracts iCalendar from paste, a chosen file, UTF-16 calendar bytes, an email `text/calendar` part, a zip of those calendars, a PDF that embeds an iCalendar, or an Office Open XML document whose visible text contains an iCalendar. Image OCR and AI parsing of confirmation prose remain out. Android Pixel 8 rehearsed choosing a local `.ics` file and an `.eml` with a calendar part; zip, PDF, and Office extractors were not rehearsed on device in this pass. iOS was not rebuilt.
