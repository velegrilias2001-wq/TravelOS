...
- Day clock V1 is implemented on this branch: when exactly one TripDay is assigned to a destination with a valid IANA timezone and that timezone’s local date is the day’s date, Companion uses that city as the clock. Two cities claiming today, an unassigned day, or a city without a timezone keep the trip-level or device fallback. Destination order is never a clock. Home still resolves featured-trip phase from destinations only. Automated tests exist. No device rehearsal.
...
- Day → Destination assignment is implemented for Plan and Companion. Remaining gaps are a picker-supplied place ID and a destination-currency source. Home still does not load days when choosing the featured trip.
- Companion V2 stop-boundary refresh decisions, lived progress, and external live-data layers. Destinations may store an IANA timezone from catalogue, traveler, or a future provider field; the installed picker still does not return one. Two assigned cities claiming today still fall back instead of guessing.
