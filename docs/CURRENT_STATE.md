...
- Elevation Program Wave 1 is in progress on this branch: deeper ink/teal palette, shared motion timing + `RiseIn`, editorial Home (Create / Help me decide / Import doors + what-matters-now hero), stronger local-dev copilot system prompt, and Profile → TravelOS Copilot health probe against `GET /ai/tools`. AI remains loopback-only and cannot mutate SQLite. Waves 2–4 (Create Trip delight, Companion/Plan AI density, Discover explain polish) are sequenced in `docs/ROADMAP.md`.
