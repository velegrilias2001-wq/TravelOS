...
## Recovery and editor safety continuation — 2026-09-09

Latest evidence supersedes the older R1 progress notes below; neither the full R1 nor the full R2 acceptance matrix is complete.

### Final local hardening pass — 2026-09-09

- Successful restore now clears the session trip list, Discover Brief and chat transcript, advances a session-only generation and remounts retained routes. SQLite remains authoritative. Superseded list reads cannot republish pre-restore records; a late pre-restore chat response cannot repopulate the cleared transcript. Notification reconciliation coalesces overlapping requests but performs a fresh pass when requested during old work; its post-commit failure produces reopen guidance rather than a replacement retry.
- Profile serializes export and picker/confirmation/restore actions with a synchronous operation gate. The restore overlay prevents navigation during the workflow. Native Android picker cancellation returned to an interactive Profile without writing data or leaving a busy overlay.
- Create Trip, Trip Details, Travel Book and inline FX now use a shared route-removal guard. Unchanged routes remain unobstructed; changed routes require explicit discard; saving blocks removal; successful creation has an explicit committed-exit path. Retained tab changes do not discard drafts or demand unnecessary confirmation. Travel Book hydration no longer overwrites a dirty draft on workspace refresh.
- Android: Trip Details dirty retained-route/parent Back required discard. Travel Book retained its unsaved summary across Plan/tab return and warned on parent Back. Expense validation rejected an empty title, retained a changed draft and warned on Back; discard did not create an expense. A synthetic EUR 100 planned budget saved. Travelers' final safe-area X was below the system bar and closed an unchanged modal without a warning. Missing local photo URIs displayed `Photo unavailable on this device`; no URI was removed or invented. Memories/deletion copy now distinguishes JSON data backup from unavailable photo-file backup.
- An isolated native restore using the production runtime remounted the test screen and verified a newer session generation plus agreement between the restored SQLite trip and session list. Read-only checks first proved every affected table was either empty or owned by the exact synthetic fixture. Cold relaunch showed the restored trip; SQLite **23**, persistence self-test **PASS**, and bootstrap **ready** were logged. This uses the existing development binary with updated JS, not a freshly built release APK or iOS verification.
- **442 app tests pass**, including six session/concurrency tests, a config opt-out regression and a 1,000-additional-stop archive round-trip (1,428,459 UTF-8 bytes, approximately 180 ms in the standalone Node SQLite run). The latter verifies ordered content, not physical-device performance or peak RAM. Full 8 MiB boundary/escaping tests remain in place.
- Final cleanup deleted only `final-hardening-only-20260909`; read-only counts confirmed all 21 affected canonical/import tables empty in the isolated app. The temporary route and inspection XML were removed. No real/pre-existing data was deleted and no external sharing occurred.
- Final checks after harness removal: `npm test` **442/442**, `npx tsc --noEmit` and `git diff --check` passed; lint **0 errors / 49 existing warnings**. Verification branch: `docs/expo-sqlite-rehearsal-7-15`. This is a bounded hardening checkpoint, not completion of the full R1/R2 acceptance matrix.
- Security correction: the custom Expo config loader previously ignored `EXPO_NO_DOTENV`, so earlier config evaluation did not guarantee the requested no-read boundary. It now honors the flag before any env-file filesystem access, with a sentinel test. No secret values were printed or copied. Subsequent Metro verification used the opt-out and a non-secret config placeholder; the installed native Maps configuration was not rebuilt or changed. Normal config loading without the opt-out is unchanged on both platforms.

### Create Trip / inline FX native follow-up — 2026-09-09

- Existing isolated Android development build received the current Metro JS bundle on port 8082, with secret-file loading disabled. No new APK, dependency, schema or production-code change in this verification pass.
- Create Trip: native date-picker cancellation preserved the dates; previous-step navigation and Keep editing preserved the selected intent. Explicit discard left zero trips. A subsequent normal form submission created one synthetic trip, two canonical days and a EUR 100 budget, then opened Trip Copilot without a discard prompt.
- Inline FX: incomplete input produced a validation alert; the USD draft survived Plan/tab return. Explicit discard wrote no rate. Saving a deliberately synthetic USD→EUR 0.9 rate dated 2026-09-09 persisted it and reset the dirty baseline; Back no longer demanded discard. This is test data, not a market-rate assertion.
- Cold force-stop/relaunch logged SQLite **23**, persistence self-test **PASS**, bootstrap **ready**. Normal UI navigation Home → Trips → Companion → More → Budget showed the same trip, upcoming truth, EUR 100 planned/remaining, zero spent and saved FX rate. Plan showed the two dates/zero moments; Map and Bookings showed their corresponding empty states. This is a focused traveler-style walkthrough, not exhaustive feature/provider verification.
- The emulator displayed **System UI isn't responding** during the relaunch attempt. Wait was selected; Android returned to its launcher and TravelOS was reopened without data reset. Subsequent navigation completed, but delayed/transitional UI snapshots occurred. Cause is not established, and this run is not a clean cold-start/performance acceptance result. DateTimePicker also emitted an existing `onChange` deprecation warning; cancellation worked.
- Deleted only the exact UI-created fixture `1112e278-1fb3-496b-89cf-1e2aecf80ad6` through TripService. Native read-only counts then confirmed zero trips, days, destinations, budgets, expenses and FX rates in the initially empty isolated app. The original `com.travelos.app` was untouched. Temporary harness and inspection artifacts were removed.

Outstanding: broader per-editor database-save-failure/nested-picker cases, rapid actions, iOS dismissal, large-archive Android memory/time, and authorized share-target delivery. Notification scheduling success in this pass is not proof of real reminder delivery. R3 accessibility, R4 release/provider safety and physical devices (including a repeat stable cold-start run), R5 traveler pilot, and R6 evidence-led expansion remain separate gates. No migration or dependency change was introduced.

### Earlier modal and file-workflow checkpoint

- Backup preflight now rejects unsupported durable fields, enum values, malformed currency codes, clocks and recorded timestamps, and conflicting Memory/Runtime day–stop pairs before replacement. Supported legacy booking wall/absolute strings remain unchanged. Historical descriptive `contract` metadata is not restore authority. The field allowlists must evolve with the domain; this is not exhaustive historical-database compatibility proof.
- A committed restore followed by a failed trip-list refresh is reported as restored data requiring app reopening, not as a failed replacement to retry. SQLite failures still propagate. Wider workspace/notification reconciliation and concurrent restore UI actions remain open.
- Actual Android Profile export opened the system share sheet (sharing was cancelled, no external recipient). Its exact synthetic JSON was copied to Downloads for the native picker; Profile confirmation/replacement succeeded. After force-stop, development-client reconnection and reopening, the original title, stop ID, booking link and USD currency survived. This verifies local picker recovery, not delivery to/retrieval from a cloud share target.
- R2 modal close protection now covers Bookings, Accommodation, planned Budget, expenses, Travelers and Memories. It compares local form drafts, blocks exits while saving, asks Keep editing/Discard only when changed, and leaves successful saves on their existing direct close path. The planned-budget currency-settings exit is guarded too. No autosave or second durable store was added.
- Native review found Accommodation's close control overlapping the Android status bar. A shared modal-local SafeAreaProvider/SafeAreaView boundary now wraps Accommodation, Travelers and Memories. Accommodation's corrected X and Memory scrolling/Save were verified; the final Travelers wrapper still needs its own native follow-up. Booking/Budget close controls have explicit accessibility labels. See `EDITOR_SAFETY_MATRIX.md` for per-flow evidence and gaps.
- **434 app tests passed**, TypeScript and whitespace checks passed. No schema, migration or dependency changes. Lint remains **0 errors / 49 pre-existing warnings**. Runtime verification used the existing isolated Android development build; no new release APK/iOS build is implied.
- Cleanup confirmed zero trips in the previously empty isolated app after deleting only the known fixture. Its note, test export, hash-matched picker copy, Downloads copy, temporary route and local inspection files were removed. Another pre-existing picker cache file was left untouched. The original `com.travelos.app` database was not inspected or changed.

Next: finish restore reconciliation/concurrency/large-archive and missing-media checks; then route-level editor exits (Create Trip, Trip Details, Travel Book), expense/picker/failure cases and the broader accessibility matrix. Physical-device, iOS, proxy release controls and traveler pilot remain separate gates. Current status is a broad working native prototype undergoing hardening, not public-release readiness.

## R1B.1 — Consistent export snapshot (2026-09-08)

R1B.2 progress (2026-09-08): the export writer now validates the exact serialized JSON against the existing restore preflight and 8 MiB UTF-8 limit before any filesystem write. Formatting/escaping count toward the limit; oversized output is rejected, not truncated. Filenames use a locally generated timestamp, not supplied export metadata. Profile explicitly discloses plaintext/private data, the size limit and non-portable photo references. Five new tests cover unchanged valid output, exact-limit/over-limit bytes, multibyte/escape overhead, bad links and legacy v1 files without packing using preserved wall/absolute booking timestamps. **423 app tests passed**. Android native file write/read and no-write rejection checks passed with synthetic data; Profile disclosure was visible. Synthetic file and temporary route were cleaned. This does not complete exhaustive field semantics, large-archive benchmarks, full successful Profile share/picker restore, or post-restore state reconciliation.
...
- Plan Assist themes no longer inherit destination-center coordinates, including stale session candidates. Existing saved stops are untouched.
- Map requests follow explicit trip/day IDs and adjacent canonical stop order. Unmapped stops break the chain. The six-leg cap/partial coverage is disclosed; stale async results cannot represent a newer workspace/day.
- Restore preserves device AI/notification preferences. AI starts fail-closed; bootstrap awaits preferences; serialized reads/writes prevent stale reads overriding disable. Settings do not wait for server health. Scrollable privacy copy describes transmitted context and does not promise provider zero retention.
- P0 layout work is preserved with Home action-card wrapper sizing and React Compiler-compatible animation writes. No schema, dependency, credential or provider configuration change.
- Checks: app **402/402**, server **61/61**, TypeScript and whitespace checks passed; lint **0 errors, 49 warnings**.
- Android `emulator-5554`: x86_64 development build compiled and installed side-by-side as `com.travelos.app.hardening`. Logs confirmed SQLite **23**, persistence self-test PASS and bootstrap ready. Home cards/tab clearance and AI privacy controls were inspected. AI-off survived cold relaunch with an unavailable local AI server. Original `com.travelos.app` was not replaced/reset.
- Android regression completed on 2026-09-08 using a temporary development harness and a clearly labeled synthetic trip: two real restore-runtime calls preserved AI-off, days and stop content; reorder persisted without loss; route generation respected unmapped gaps and day boundaries; accepting a stale theme left location unset. Native Plan loaded all six moments; a further reorder through its controls appeared in Map. Today showed two mapped points; View all showed four, excluding two unlocated moments. The first harness attempt used invalid zero-based positions; correcting the fixture to the existing one-based contract passed without a product-code change. The exact test trip and its children were deleted, cleanup was confirmed, and the temporary route was removed before commit.
- Limits: this was an Expo SQLite/runtime and native-screen rehearsal, not a system file-picker restore or live-provider route certification. The isolated package does not establish production Maps-key authorization; routes correctly reported unavailable. Hosted AI, live routes, iOS, large-font and full offline journeys were not re-verified.
- Earlier P1 observations establish visible controls, not every end-to-end mutation. Public-release gates remain proxy access/abuse/cost controls, draft safety, backup/recovery and media portability, accessibility/localization and traveler trials. See `PRODUCT_EXPERIENCE_AUDIT_2026-09-06.md`.
...
- Android native rebuild + Expo SQLite 16–17 rehearsal is recorded on this branch: a Pixel 8 development-build rebuild of `com.travelos.app` autolinked `expo-network` 57.0.1 and upgraded the installed database from live `user_version` 15 to 17. Development logs reported Memory foreign keys `stop_id,day_id,trip_id`, `Persistence self-test: PASS`, and `Bootstrap ready`. Existing trips survived (Home still showed Nagawa as HAPPENING NOW with 5 trips / 0 completed). Companion and Map opened on Nagawa without a crash; Map still named stops without saved coordinates. Airplane-mode offline copy was not rehearsed. iOS was not exercised.
- Phase 1 remaining trip-core V1 is implemented on this branch: migration version 16 persists destination `place_id` / `timezone_source`, traveler FX rates with as-of provenance, and an optional trip-owner membership. Timezones are accepted only from a provider result, catalogue handoff, or traveler IANA choice. Foreign expenses enter accounting totals only through an explicit traveler rate. Booking amount and currency must be saved together. Archive remains reversible status; delete remains irreversible on-device. Automated tests exist. An Android Pixel 8 rebuild on 2026-09-03 ran version 16 as part of the live 15→17 upgrade. FX, place-ID, and owner UI were not separately rehearsed. iOS was not exercised.
- Day clock V1 is implemented on this branch: when exactly one TripDay is assigned to a destination with a valid IANA timezone and that timezone’s local date is the day’s date, Companion uses that city as the clock. Two cities claiming today, an unassigned day, or a city without a timezone keep the trip-level or device fallback. Destination order is never a clock. Automated tests exist. An Android Pixel 8 pass on 2026-09-03 opened the isolated Lisbon trip as ON THE JOURNEY with hero LISBON, Day 1 of 1, and `13:32 in Europe/Lisbon`. Nagawa still showed CITY NOT SET FOR TODAY.
...
- Stop lived phase V1 is implemented on this branch: Companion can mark a planned stop done or skipped without rewriting Plan times. Delayed is derived when a timed stop’s start has passed and the traveler has not marked it. `TripRuntimeState` is written only as a pointer to the last explicit lived stop. Migration version 17. Automated tests exist. An Android Pixel 8 rebuild on 2026-09-03 ran version 17. A later pass marked isolated Lisbon `RehearsalCoffee` Done; Companion showed Undo and said marks do not change the saved plan.
- Map day framing + directions V1 is implemented on this branch: Map frames the assigned city and that day’s mapped stops when Companion has a display day. View all still fits every saved destination and stop coordinate. Directions open Apple Maps or Google Maps on the saved pin only. Accommodation coordinates are still absent. Automated tests exist. No device rehearsal for day framing alone.
- Real routes / ETAs V1 extends Map with provider polylines and walking ETAs between consecutive mapped stops via `/geo/directions` (see checkpoint above).
...
- Local-dev AI boundary V1 is implemented on this branch: advice cannot write SQLite or bookings; unreachable AI degrades; automated tests exist. Preview/production may set HTTPS `EXPO_PUBLIC_TRAVELOS_AI_URL` to the TravelOS proxy (`resolveTravelOsAiBaseUrl` / `isAllowedAiBaseUrl`). Loopback remains the local-dev default. Provider API keys stay on `server/` only — never in `EXPO_PUBLIC_*`.
- Local-dev AI provider abstraction V1 is implemented on this branch: the `server/` package resolves `AI_PROVIDER` / `AI_CHAT_MODEL` / `AI_EMBEDDING_MODEL` (with `OLLAMA_*` aliases), exposes a single `createAiProvider()` for chat/embed, keeps a TravelOS copilot system prompt file, lists real vs `not_configured` tools at `/ai/tools`, and supports `openai_compatible` for hosted OpenAI-compatible endpoints. Feature flags: `AI_ENABLED`, `AI_SEMANTIC_SEARCH_ENABLED`, `AI_VISION_ENABLED`, `AI_RERANK_ENABLED`, `AI_VOICE_ENABLED=false`. See `server/.env.example`.
- Plan lived badges V1 is implemented on this branch: Plan shows Done or Skipped from explicit lived-stop states without rewriting saved times. Delayed is not a Plan badge. Marks remain Companion actions. Automated tests exist. An Android Pixel 8 pass on 2026-09-03 showed Plan DONE on unmapped `RehearsalCoffee` after a Companion Done mark; the moment stayed untimed.
- Create Trip traveler timezone V1 is implemented on this branch: Create Trip can set or clear an explicit IANA timezone with `traveler` provenance. Replacing the map pin keeps a traveler timezone when the picker still has none. Coordinates are never used to guess a zone. Automated tests exist. An Android Pixel 8 pass on 2026-09-03 showed Trip Details Porto as `Europe/Lisbon` / Saved timezone, and Coimbra as Unknown / Not guessed from the map pin.
- Booking sheet sticky footer and currency fix are implemented on this branch: the Add/Edit booking save button is outside the ScrollView with safe-area bottom padding so it clears Android gesture nav. Selecting a booking type prefills an empty name with the type label. `resolveBookingCurrencyCode()` omits currency when amount is empty. Delete booking rows expose `Delete {title}` accessibility labels. Automated tests cover the currency helper (324 passing).
- Phase 5 visual/function matrix remainder was closed on Pixel 8 on 2026-09-03 for the isolated Lisbon trip: booking save (Ferry prefill, no keyboard), booking delete, stop delete (`RehearsalCoffee`), and trip delete confirmation shown then cancelled. Lisbon trip remains on device without the rehearsal moment or booking.
- Pass 1 functional motion is implemented on this branch: tab icon one-shot scale on select (only when a tab becomes focused, not on cold start), `PressableScale` on Create Trip and booking save (`alignSelf: 'stretch'` so CTAs keep full width), Plan Done/Skipped `FadeIn`, shared `useReduceMotion`. No Lottie.
- Pass 2 functional motion is implemented on this branch: Discover match cards stagger the first six items, wishlist bookmark pulse only when a save goes inactive→active, Companion/Map offline notices slide in when reachability is known, and World filter list fades on Planned/Lived/All. A code audit confirmed those call sites. An Android Pixel 8 smoke on 2026-09-03 opened Home, Discover brief, World filters, and Nagawa Companion without crash.
- Light haptics V1 is implemented on this branch: `expo-haptics` 57.0.2 with `playSelectionHaptic` / `playLightImpact` that skip when Reduce Motion is on and fail closed if the native module is missing. Wired on tab select, Create Trip / booking save press, Companion Done/Skip pills, and Discover wishlist toggle. An Android development-build rebuild on 2026-09-05 autolinked `expo-sharing` and `expo-haptics`; Profile export opened the system share sheet. Emulator haptic *feel* remains limited on virtual devices.
- Unused Expo starter cleanup V1 is implemented on this branch: removed the unused template island (`animated-icon*`, `app-tabs`, `collapsible`, `external-link`, `hint-row`, `web-badge`, `themed-text`/`themed-view`, starter `constants/theme`, color-scheme hooks, `global.css`), orphan Expo images (`expo-logo`, `logo-glow`, badges, `react-logo*`, `tutorial-web`, `tabIcons`), and the direct `expo-web-browser` / `expo-symbols` app dependencies. `expo-symbols` may still install transitively via `expo-router`. Kept product `ui/screen`, `utility-screen`, `native-date-time-fields`, `@/theme`, and `app.json` icons/splash. Root splash remains `expo-splash-screen` only.
- Phase 5 polish V1 close is implemented on this branch: accessibility labels and ≥44pt targets on Home, World filters, Discover find chips, Memories, Travel Book, and Travel DNA; shared `EmptyState` / `InlineError`, `LocalImage`, `confirmDestructive`, and `MIN_TOUCH_TARGET`; locale-aware date/currency formatting via `locale-format` (English copy, device locale numbers/dates; RTL deferred); imagery contract in `docs/IMAGERY.md` (local traveler media only); written performance budgets below. iOS layout/QA remains last.
- Phase 6 local export + EAS profiles V1 is implemented on this branch: Profile → Export local backup writes `travelos.local-export.v1` JSON under `travelos/exports/` and shares it when `expo-sharing` is available. Export is read-only for SQLite and omits photo bytes. `eas.json` defines development/preview/production profiles. Automated tests cover the export document contract.
- Phase 6 local restore V1 is implemented on this branch: Profile → Restore local backup picks a JSON file, validates `travelos.local-export.v1`, confirms a destructive replace, then atomically wipes and reloads trips, travelers, Travel DNA, saved places, and trip-owned records while preserving exported IDs. Photo bytes are not restored; import queues are cleared. Trip list rehydrates from SQLite after restore. Automated parse and persistence tests exist. Device restore picker smoke was not re-run in the same pass as the Android sharing rebuild.
- Performance budgets (Phase 5 V1, Pixel 8 debug target): cold open to interactive under 3s; tab switch without multi-second blank; Discover results and World map must not invent remote imagery or markers; list scroll should stay usable without forced virtualization until measured jank. Continuous monitoring stays Phase 6.
- Parked remaining work is recorded on this branch: iOS rebuild (last). Real routes/ETAs V1, Discover rerank V1 (no BGE), confirmation-photo OCR V1, picker/Time Zone enrichment V1, and Trip notifications V1 are no longer parked. Pass 1–2 motion and Phase 5 visual matrix are no longer parked.
- A 2026-09-03 Pixel 8 pass closed remaining import-extractor, wishlist, journey Create Trip, multi-destination, and Day → Destination device rehearsals on the installed `com.travelos.app` client. Import zip/PDF/Office/image files opened HIGH CONFIDENCE · PENDING reviews without writing bookings. Wishlist saved and then removed Slow days in Porto. Lisbon and Porto prefilling Create Trip with both cities; Trip Details then added Coimbra through the native location picker. Plan assigned Lisbon to Day 1. An isolated `Lisbon` trip (3 Sept 2026) remains on device with owner Alex; the later Phase 5 pass removed the rehearsal moment and booking. iOS was not exercised.
- Expo SQLite Android rehearsal 7–15 is implemented on this branch: the installed Pixel 8 `com.travelos.app` client upgraded through migrations 7–15, then a later same-day native rebuild continued through 16–17. Live `PRAGMA user_version` is 17 with Memory `trip_id` / `day_id` / `stop_id` foreign keys. The first 7–15 upgrade failed because v15 dropped `memories` while v7 itinerary unlink triggers still pointed at that table; the rebuild now drops those guards first. Existing trips remained listed. iOS was not rehearsed.
- Lint / baseline CI V1 remains on the parent history: committed Expo SDK 57 `eslint-config-expo` flat config, a non-interactive `eslint .` lint script, and a GitHub Actions workflow that runs `npx tsc --noEmit`, `npm test`, lint, and the AI server tests on Node 22. Existing React Compiler lint findings stay warnings so the gate does not require rewriting screens. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); branch `docs/expo-sqlite-rehearsal-7-15` still has no upstream, so the workflow has not run on GitHub.
- Phase 6 credentials / versioning / EAS link V1 is implemented on this branch: root `.env.example`, `app.config.js` loads `.env.local` without printing secrets, `android.versionCode` / `ios.buildNumber` in `app.json`, production `autoIncrement: true` in `eas.json`, and Expo project `@velegris/TravelOS` (`extra.eas.projectId`). Store signing and the first EAS cloud build remain operator steps.
- Elevation Program Wave 1 is implemented on this branch: deeper ink/teal palette, shared motion timing + `RiseIn`, editorial Home (Create / Help me decide / Import doors + what-matters-now hero), stronger local-dev copilot system prompt, and Profile → TravelOS Copilot health probe against `GET /ai/tools`. AI cannot mutate SQLite.
- Elevation Program Wave 2 is implemented on this branch: progressive Create Trip (Where → When → Shape) with existing destination/traveler persistence unchanged.
- Elevation Program Wave 3 is implemented on this branch: shared free-time copilot card/hook, Companion Active “Ask TravelOS” for the next verified free-time gap (display-only), and Plan provenance (`provider · model`). Android emulator smoke opened Companion on the featured Japan trip; with no verified timed gaps the copilot card correctly stayed hidden.
- Elevation Program Wave 4 is implemented on this branch: Discover results retrieve/explain polish — semantic provenance, calm unavailable degrade, explain spinner + Ask again / Try again, and `provider · model` on grounded catalogue paraphrases. No rerank install. Still grounded-only; nothing writes SQLite from AI. Android emulator smoke: Discover → Find destinations → Ask TravelOS on Barcelona showed loading, then `FROM THE CATALOGUE` with `ollama · qwen3:4b`, paraphrase guard, and Ask again.
- **PWA parity Wave D V1** is implemented on this branch: trip packing checklist (migration 22 `packing_items`, traveler-authored only, % = packed/total, More → Packing); Home readiness glance for up to three upcoming trips from countable checklist facts; World footprint country count from lived places with explicit `countryCode` only. Packing is wiped on local restore but not yet in the export contract. Share snapshot (δ2) deferred. Automated packing/footprint tests exist. No device rehearsal.
- **Travel Chat V1 (Wave E ε1)** is implemented on this branch: Home / doors → `/travel-chat` session transcript (Zustand only). `POST /ai/chat` runs grounded discover retrieve then a structured reply; destination cards resolve only from catalogue identities; Confirm prefills `/new-trip` and never writes SQLite from the model. Tool allow-list + invented-identity filters covered by server tests. Hosted Android preview smoke 2026-09-06 verified Chat → Confirm Vienna → Create Trip against Render/`openai_compatible`.
- **Trip Copilot V1 (Wave E ε2)** is implemented on this branch: More → Trip Copilot proposes readiness deep links, Plan Assist accept→`addStop`, display-only free-time advice, and an Import review door for pending claims. No silent SQLite writes. Automated proposal-selection tests exist. No device rehearsal.
- **Absolute Travel Intelligence T0–T4 (this branch):** HTTPS TravelOS AI proxy URL policy + device kill-switch (`ai_preferences` v23); Home decide/organize Greek doors; Trip Copilot evidence/packing; confirm-gated DNA reflection from Brief/trip choices; Companion Day→Destination assign nudge when city unset; More → Share trip snapshot (non-secret text via system share). Automated tests cover DNA reflection + share text.
- **Travel Intelligence 10/10 U1–U5 (this branch):** First-run Decide/Organize coach; empty Home two primary doors + Discover tertiary; Create Trip → Trip Copilot; Chat constraint chips + in-thread Discover compare; Copilot Build queue (≤3) + packing template Accept; packing in local export/restore; DNA reflection after Plan Assist accept + Profile “Προτάσεις από επιλογές σου”; Greek core surfaces; `scripts/smoke-decide-organize.sh`.
- **Hosted TravelOS AI (U2 operator, 2026-09-06):** Render Free deploys `server/` with `AI_PROVIDER=openai_compatible`, OpenAI `gpt-4o-mini` + `text-embedding-3-small`, and a regenerated Discover corpus artifact (`text-embedding-3-small` / `contentHash bce5bc94da21a457`). EAS preview sets `EXPO_PUBLIC_TRAVELOS_AI_URL=https://travelos-lckj.onrender.com`. Preview APK `71b6c20c-…` (commit `e90014c`) emulator smoke: TravelOS AI Ready; Chat grounded Confirm → Create Trip → Copilot; Packing Accept ×3. Geo/OCR Google keys on the proxy, Play AAB, and iOS remain open. Free Render may cold-start after idle.
- Design Elevation pass 1 (Companion Active) is implemented on this branch: honest day progress from settled stop phases, NOW/NEXT focus badges with RiseIn, richer linked-booking truth card (status/type/provider/code only when saved), and a journey timeline rail with settled checkmarks and muted later moments. No invented airports, routes, or booking fields. `npx tsc --noEmit` passed; emulator UI smoke was blocked this pass by the Expo development-client launcher / hung `uiautomator` (not by a Companion compile error).
- Design Elevation passes 2–6 are implemented on this branch: Bookings route-card hierarchy; Create Trip sticky Continue/Create footer; Discover overlap sheet + motion; Home denser in-hero greeting (stats hidden when featured); Plan horizontal day strip from real TripDays. No invented destinations, airports, or booking fields.
- Plan Assist V1 is implemented on this branch: empty Plan days show deterministic theme-moment suggestions from trip intent/pace, Travel DNA interests, and optional catalogue fit when the day’s assigned city matches a grounded catalogue record by **exact coordinates** (never by title). Accept writes a real TripStop via `addStop` with provenance in `notes`; no POIs are invented; suggestions stay session-only until accept. Automated tests exist.
- Seed from materials V1 is implemented on this branch: Import accepts document/text notes (txt/PDF/Office when no embedded iCalendar) into `trip_seed` / `itinerary_line` review claims (migration 18). **Start Create Trip** acknowledges a seed and prefills `/new-trip`; itinerary lines are mark-reviewed only — never silent Trip/Stop writes. ICS booking accept unchanged.
- Trip Theme V1 is implemented on this branch: optional `trips.theme_pack_id` with curated packs by confirmed `countryCode`; assigned on Create Trip; Home featured hero accent + mood eyebrow and Companion mood line use the pack. Cosmetic only — no invented destinations.
- **Picker Time Zone enrichment V1** is implemented on this branch: after `expo-location-picker` returns coordinates without an IANA timezone, the native client POSTs to the local-dev server `POST /geo/timezone`, which calls Google Time Zone API with a dedicated `GOOGLE_TIMEZONE_API_KEY` (never `GOOGLE_MAPS_API_KEY`). Success persists `timezone` + `timezoneSource: 'provider'`. Missing key, offline server, or non-OK Google status fail closed (unknown stays unknown; traveler can still set IANA manually). No schema migration. Automated tests exist. Device enrichment smoke requires the local server + key. Production cloud proxy remains Phase 6.
- **Trip notifications V1** is implemented on this branch: Profile → Trip notifications opt-in toggle (migration 19 `notification_preferences`). Local `expo-notifications` reminders fire 15 minutes before timed stop starts when a day/trip has a canonical destination IANA timezone. Untimed, settled done/skipped, completed/archived trips, missing timezone, and denied permission fail closed. Reconcile on bootstrap, foreground, trip save/delete, workspace mutations, and settings toggle. No push/FCM. Native rebuild required for the notifications module. Automated planner tests exist.
- **Real routes / ETAs V1** is implemented on this branch: local-dev `POST /geo/directions` proxies Google Directions with dedicated `GOOGLE_DIRECTIONS_API_KEY`. Trip Map loads walking legs between consecutive mapped stops, draws provider polylines, and shows duration/distance text. Missing key/offline/non-OK fail closed — no invented times. System Maps deep links remain. Automated tests exist.
- **Confirmation-photo OCR V1** is implemented on this branch: Import images without an embedded iCalendar may call `POST /import/ocr-extract` (Cloud Vision via `GOOGLE_VISION_API_KEY`, gated by `AI_VISION_ENABLED`). OCR text becomes pending import claims only — never silent Booking/Trip writes. Fail closed when Vision is off or unreadable. Automated tests exist.
- **Discover rerank V1** is implemented on this branch: optional listwise reorder of already-retrieved grounded identities via `POST /ai/discover-rerank` when `AI_RERANK_ENABLED=true` (qwen; no BGE install). Invented identities and provider failures leave retrieve order unchanged. Automated identity guards exist.
- Phase 6 Android EAS preview build V1 is implemented on this branch: `GOOGLE_MAPS_API_KEY` is an EAS project env secret for development/preview/production; build profiles set `environment`; Expo remote Android keystore was created; first preview APK finished 2026-09-05 (`ca98eafd-e956-4e05-9a62-78aeddfdfa94`); second preview APK the same day (`817e2c05-c1b6-4cbd-9160-fe9e2987297f`, commit `a36f2b5`) includes tab-bar safe-area. Play Console tracks remain open. Preview APK signing SHA-1 for Maps Android-app restriction: `16:CB:EE:C7:E9:A5:6C:DB:7C:B1:A1:BF:2F:0A:8C:1D:9B:7F:52:AA` with package `com.travelos.app` (re-verified from APK cert digest on 2026-09-06; operator confirmed already present on the Maps key). Emulator preview smoke the same day: installed `817e2c05-…` after uninstalling the debug client; Create Trip location picker returned Lisboa with live Places suggestions and Google Map; Trip Map rendered tiles + Lisbon pin; World showed 1 planned mapped place.
- Android tab bar safe-area V1 is implemented on this branch: main tabs and Trip Space tabs use bottom system insets so labels/icons clear the gesture / 3-button navigation bar. Verified in native rebuild and included in preview APK `817e2c05-…`.
- UX Fix Wave P0 is implemented in the current working tree and was verified on a Pixel 8 Android development build and standalone EAS preview APK on 2026-09-06 with 3-button navigation: main and Trip Space tab labels remain complete and clear of system navigation; scroll content clears the tab bar; Home glance labels no longer collapse vertically; Discover `Flexible` remains one line; Plan Start/End fields render as compact `Add time` controls; and the World lived-empty message stays inside the top facts card instead of covering it. The native debug build compiled and launched. EAS preview build `78a537ed-22cd-40b0-bfe4-a5112c498057` installed and cold-launched without Metro, reported `Bootstrap ready`, retained onboarding completion across relaunch, and passed focused Home, Discover, and World layout smoke. Play remains operator-paused.
- UX Fix Wave P1 hybrid audit is complete in the current working tree. On standalone preview `78a537ed-22cd-40b0-bfe4-a5112c498057`, an empty local install exposed Decide, direct Create Trip, and Import; the Profile kill-switch disabled network AI while a real Athens provider result still flowed through `/new-trip`; an isolated one-day `Athina, Greece trip` showed Plan Assist beside manual `Add a moment`, compact optional time controls, Packing Accept rows beside manual free-text add, the complete More hub, and an explicit Companion Day→Destination assignment nudge. Cold hosted-AI health recovered to Ready without blocking manual paths. Existing automated tests cover AI failure/degrade and import acceptance without AI; airplane-mode and import-file device flows were not repeated on this preview. The test-only trip remains in the isolated emulator database. No new architecture or product feature was required. Play remains paused by operator instruction.
...
- Phase 6 credentials and versioning discipline V1 is implemented on this branch: committed root `.env.example` (names only), `app.json` `android.versionCode` / `ios.buildNumber`, EAS production `autoIncrement: true`, and export `appVersion` from `expo-constants`. Secrets stay in uncommitted `.env.local` / `server/.env`. `app.config.js` loads `.env.local` for config evaluation without printing values.
- EAS project link V1: logged in as `velegris`, project `@velegris/TravelOS` at https://expo.dev/accounts/velegris/projects/TravelOS with `extra.eas.projectId` set. Store signing / first cloud build not yet run.
...
- Multi-destination Trips now persist an optional Day → Destination ID. When that city has a valid IANA timezone and is uniquely today in that timezone, Companion uses it as the clock for the current date, current day, and NOW/NEXT. Runtime calculations otherwise still use a destination timezone only when every relevant saved destination has the same valid IANA timezone; otherwise the resolver reports an explicit device-calendar fallback. Destinations may store an IANA timezone from a provider result, catalogue handoff, or traveler choice, with `timezoneSource` provenance. The installed picker still does not return a timezone itself; local-dev enrichment via `POST /geo/timezone` (dedicated Time Zone API key on the server, never the Android Maps key) can fill `provider` timezone after pick. Fail closed when the server or key is unavailable. Traveler override remains. Two assigned cities claiming today are not resolved by destination order.
...
- An Android Pixel 8 Expo SQLite rehearsal on 2026-09-03 upgraded the installed development database through versions 7–15, then a native rebuild the same day continued through 16–17. Live `PRAGMA user_version` is 17, and `memories` has declared `trip_id`, `day_id`, and `stop_id` foreign keys. Equivalent iOS rehearsals, including migration version 3, are still outstanding.
...
- Create Trip requires the existing native picker and persists one or more real selected destinations, up to eight. After a city is chosen, Create Trip can set or clear an explicit IANA timezone with `traveler` provenance. Replacing the map pin keeps that timezone when the picker still has none. Trip Details uses the same picker to add another destination, replace a structured destination, or upgrade a historical name-only destination while preserving its TravelOS ID. Reorder and remove keep destination IDs; a trip that already has destinations cannot be saved with zero. Removing a destination does not delete itinerary stops, bookings, or stays; it clears Day → Destination assignments for that city.
...
- Done and Skipped badges on Plan from explicit Companion lived marks. Saved start/end times stay the plan. Delayed is not shown on Plan.
...
- The AI client resolves `EXPO_PUBLIC_TRAVELOS_AI_URL` via `resolveTravelOsAiBaseUrl`: loopback http/https for local-dev/`adb reverse`, HTTPS remote hosts for preview/production TravelOS proxy, and optional cleartext LAN only when `EXPO_PUBLIC_TRAVELOS_AI_ALLOW_CLEARTEXT=true`. Credentials in the URL fail closed. The local `server/` package defaults `PORT` to `8787` unless configured (historical Android rehearsal used `PORT=8789` + `adb reverse`).

Current limitations include limited stop-type UI, no route or transit model, no place-provider ID persisted from the picker, no automatic insertion of AI suggestions as stops, and large screen-level implementations with duplicated presentation patterns. Plan shows a restrained booking count and a contextual action for relationships resolved by exact stop ID. Done/skipped marks are still recorded in Companion, not as Plan actions. The 2026-09-02 Android rehearsal opened Plan on the existing Nagawa trip, derived a 13:00–14:30 free-time gap, received two TRAVELOS IDEAS from live Qwen, and left the itinerary unchanged.
...
- Local `server/` Express app with `/health`, `/ai/health`, `/ai/tools`, `/ai/free-time`, `/ai/discover-retrieve`, `/ai/discover-rerank`, `/ai/discover-explain`, `/geo/timezone`, `/geo/directions`, and `/import/ocr-extract`. Chat/embed go through `createAiProvider()` (`ollama` default; `openai_compatible` prepared for a dedicated endpoint). Model IDs come from `AI_CHAT_MODEL` / `AI_EMBEDDING_MODEL` (aliases `OLLAMA_MODEL` / `OLLAMA_EMBED_MODEL`). Defaults remain local candidates `qwen3:4b` and `bge-m3`, not a production architecture commitment. Whisper and a general chat UI are not activated.
- Plan and Companion Active may call free-time advice (display-only). Discover results may call `/ai/discover-retrieve` for extra grounded identities, optional `/ai/discover-rerank` when enabled, and `/ai/discover-explain` for an opt-in catalogue explanation of one existing candidate. Those paths cannot invent destinations or write SQLite. Discover results surface retrieve/explain `provider · model` provenance and a calm semantic-lane degrade when retrieve is unavailable.
...
Current limitations include no on-device model, no BGE reranker install, no silent conversion of advice into canonical stops, and optional geo/OCR Google keys not yet set on the hosted proxy. Preview/production may use a hosted `openai_compatible` TravelOS proxy; local-dev still defaults to Ollama. Discover corpus embeddings on this branch target `text-embedding-3-small` (regenerated 2026-09-06) for hosted retrieve lockstep.
...
- `EXPO_PUBLIC_TRAVELOS_AI_URL` may be loopback (local-dev) or HTTPS TravelOS proxy (preview/production). Credentials in the URL fail closed. Non-HTTPS remote hosts require `EXPO_PUBLIC_TRAVELOS_AI_ALLOW_CLEARTEXT=true`.
...
- A generate script (`npm run generate:discover-embeddings`) that writes a committed server-side artifact at `server/data/discover-corpus-embeddings.json`, keyed by grounded identity `(source, record id)` and a content hash of the current corpus document texts plus embedding-model candidate. The current artifact has 12 `text-embedding-3-small` 1536-d vectors (`contentHash bce5bc94da21a457`). The native app does not bundle or load these vectors. A Node test fails if the artifact hash drifts from the live corpus. The generate script supports Ollama or OpenAI-compatible `/embeddings` when `AI_BASE_URL` + `AI_API_KEY` are set.
...
### Discover reranking V1 (listwise, no BGE)

Implemented in code:

- Optional `POST /ai/discover-rerank` when `AI_RERANK_ENABLED=true`. Discover results may reorder already-retrieved grounded identities with qwen listwise prompts.
- Fail-closed identity application in `src/services/discover-rerank.ts` (`applyRerankedIdentities`, `reorderSemanticHitsByIdentities`): invented identities are rejected; omitted identities keep their original tail order. Disabled/unavailable/timeout leave retrieve order unchanged.
- A BGE reranker was not installed. `AI_RERANKER_MODEL` remains documented only.
- The local retrieve-then-rerank harness (`npm run benchmark:discover-rerank`) remains for measurement.

Honest read: Qwen listwise quality is uneven and can be slow; the flag defaults off. Deterministic matching stays primary.
...
- iOS bundle identifier is `com.travelos.app`. Trip Map and World already request `PROVIDER_GOOGLE`, so a future iOS rebuild needs Maps SDK for iOS enabled and the key restricted to that bundle.
- iOS build, picker behavior, device permissions smoke, and release readiness are unverified (Windows cannot rebuild iOS).
- Memory camera/photo Info.plist strings are prepared via the `expo-image-picker` config plugin; they take effect on the next native rebuild.
...
Plan lived badges V1 and Create Trip traveler timezone V1 verification on 2026-09-03 re-ran `npx tsc --noEmit` and `npm test` (323 application tests passing, including Plan badges from explicit done/skipped only, traveler IANA on Create Trip, and map-replace keeping a traveler timezone). `git diff --check` was clean. No native rebuild or device rehearsal was run.

Parked remaining work verification on 2026-09-03 re-ran `npx tsc --noEmit` and `npm test` (323 application tests passing). `git diff --check` was clean. At that date, picker/Time Zone API, routes/ETAs, notifications, Discover rerank, OCR, remaining Phase 5 visual matrix, and iOS rebuild were deferred. By 2026-09-05 the Phase 5 visual matrix was closed on device. By 2026-09-06 picker Time Zone enrichment V1 landed (local-dev `/geo/timezone`); routes/ETAs, notifications, Discover rerank, OCR, and iOS remain deferred.

Android native rebuild and Expo SQLite 16–17 rehearsal on 2026-09-03 compiled a debug `com.travelos.app` over the installed Pixel 8 client (data preserved), autolinked `expo-network` 57.0.1, and launched against Metro (`adb reverse tcp:8081`). Development logs reported `SQLite user_version 17`, Memory foreign keys `stop_id,day_id,trip_id`, `Persistence self-test: PASS`, and `Bootstrap ready`. Home still showed Nagawa as HAPPENING NOW (1–10 Sept 2026) with 5 trips / 0 completed. Companion opened ON THE JOURNEY with CITY NOT SET FOR TODAY, Day 3 of 10, and Done/Skip on a planned stop; no mark was recorded. Plan showed 10 days / 6 moments including derived FREE TIME. Bookings listed the existing Flight tokyo confirmation. More showed Trip details, 0 travelers, Budget, 0 stays, and 2 memories. Map showed Nagawa with 1 saved place and named stops without coordinates. Cold relaunch returned to the same Home counts with `user_version` 17. Airplane-mode offline copy, FX/place-ID/owner UI, and iOS were not exercised.

Phase 5 Travelers/forms remainder on 2026-09-03 opened the isolated Lisbon trip. Companion showed LISBON, Day 1 of 1, and `13:32 in Europe/Lisbon`. Travelers created adult Alex, listed IN TRIP, and opened the edit form (shared-truth note, type, names, Save changes) without saving edits. A later pass made Alex OWNER. More then showed 1 traveler. Trip Details showed title Lisbon, status Planned, Porto `Europe/Lisbon` saved, Coimbra Unknown / not guessed from the pin, dates 3 Sept 2026, EUR, Save trip details, and Delete trip. Plan created an unmapped `RehearsalCoffee` place on Day 1 (1 day · 1 moment) without inventing coordinates. Companion then marked it Done; copy said marks do not change the saved plan; Plan showed a DONE badge and kept the moment untimed. Bookings stayed empty: Add booking sheet opened, but the save control sits below the fold and emulator upward swipes returned Home. Stop delete, booking save/delete, and trip delete confirmation were not exercised. The isolated Lisbon trip, owner Alex, and Done `RehearsalCoffee` remain on device.
...
- No verified accessibility, device-offline, performance, upgrade, or destructive-migration test plan. Offline essential context V1 has Node tests for reachability and missing pins. A Pixel 8 native rebuild linked `expo-network` and opened Companion/Map; airplane-mode offline copy was not rehearsed.
- Android Expo SQLite now has a live `user_version = 17` rehearsal. iOS still has no recorded migration-3, 7–15, 16, or 17 rehearsal.
...
- Hosted preview AI (2026-09-06) uses a Render TravelOS proxy with OpenAI-compatible chat/embed. Local-dev may still use Ollama. The client never embeds provider API keys. Unreachable AI degrades. There is still no accounts/cloud sync and no Play/iOS store shipment.
...
The latest local git work is on `docs/expo-sqlite-rehearsal-7-15` through Elevation Program Waves 1–4, Wave 5 iOS config prep, Phase 6 local restore/credentials/EAS link, and Android preview cloud builds including the UX Fix Wave P0 artifact `78a537ed-22cd-40b0-bfe4-a5112c498057`. Live Android `user_version` remains 17. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); this branch tracks `origin/docs/expo-sqlite-rehearsal-7-15` and may still be ahead of remote. EAS project `@velegris/TravelOS` is linked with a remote Android keystore and finished preview APKs. Preview Maps smoke on `817e2c05-…` and focused UX P0 smoke on `78a537ed-…` passed 2026-09-06; Maps key Android-app SHA is already registered. Play tracks remain open pending the P1 hybrid/manual-path audit. iOS remains last.

2026-09-05 local release check: `npx tsc --noEmit` clean; app `npm test` 333 passing; server `npm test` 33 passing. Android rebuild the same day linked `expo-sharing` / `expo-haptics` and verified Profile export share sheet.

The immediate planned product-development sequence for **parked** items still needs a device session or iOS last: iOS rebuild. Real routes/ETAs, Discover rerank (no BGE), confirmation-photo OCR, picker Time Zone, and Trip notifications are implemented in code. Phase 5 visual/function matrix and polish V1 are closed. Phase 6 local export/restore, EAS profiles/project link, and credentials/versioning are implemented. Next operator steps are Play internal testing (optional), then accounts/sync design — not iOS yet.
