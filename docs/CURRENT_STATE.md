...
Current limitations include no Day → Destination clock, no backup/export for deletion, and no complete currency selector. Intent and pace editing exist in code; they were not re-verified on a device during this documentation pass.
...
Current limitations include no stable provider place ID in the installed picker result, no destination-currency source, and no use of a day’s assigned city as a clock. Fully timezone-aware multi-destination NOW/NEXT remains Phase 2. Provider enrichment and iOS verification remain future work.
...
- Android Expo SQLite now has a live `user_version = 15` rehearsal. Migration 16 is in code and Node tests only. iOS still has no recorded migration-3, 7–15, or 16 rehearsal.
...
- Forward-only migrations through version 16, including duplicate-day archival, Booking/Accommodation/Memory relationship cleanup, Travel DNA, trip intent/pace, saved Discover candidates, Memory day/stop foreign keys, destination place identity and timezone provenance, traveler FX rates, and optional trip-owner membership.
...
- Day → Destination assignment is implemented for Plan and Companion presentation. Remaining gaps are a picker-supplied place ID, using a day’s city as a clock, and a destination-currency source.
- Companion V2 stop-boundary refresh decisions, lived progress, and external live-data layers. Destinations may store an IANA timezone from catalogue, traveler, or a future provider field; the installed picker still does not return one. A day’s assigned city is display-only.
