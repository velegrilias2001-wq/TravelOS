...
- Parked remaining work is recorded on this branch: real routes/ETAs, notifications (unblocked for product work after Time Zone V1), Discover rerank (no BGE install), confirmation-photo OCR, and iOS rebuild (last). Picker/Time Zone API enrichment V1 is no longer parked. Pass 1–2 motion and Phase 5 visual matrix are no longer parked.
...
The immediate planned product-development sequence for **parked** items still needs a device session, a real provider, or iOS last: real routes/ETAs, notifications (Time Zone enrichment V1 is in), Discover rerank without installing BGE, confirmation-photo OCR, and iOS rebuild. Phase 5 visual/function matrix and polish V1 are closed. Phase 6 local export/restore, EAS profiles/project link, and credentials/versioning are implemented. Next operator steps are Play internal testing (optional), then accounts/sync design — not iOS yet.
