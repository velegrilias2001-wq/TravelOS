...
- Booking sheet sticky footer and currency fix are implemented on this branch: the Add/Edit booking save button is now outside the ScrollView so it remains visible regardless of sheet content height. `KeyboardAvoidingView` wraps the sheet on iOS. The currency-without-amount save bug is fixed (empty amount no longer sends a stale currency code). All 323 automated tests pass and tsc is clean. On-device emulator text-entry automation is blocked by the Android soft keyboard covering the save button and preventing adb from interacting; the code fix is verified through tests only. Stop/booking delete and trip delete confirmation were not rehearsed.
