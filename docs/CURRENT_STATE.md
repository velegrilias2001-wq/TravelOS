...
- **Absolute Travel Intelligence T0–T2 (in progress on this branch):** production-ready AI proxy URL policy (loopback + HTTPS TravelOS proxy; optional cleartext LAN flag); client kill-switch via migration 23 `ai_preferences` + Profile → TravelOS AI; Chat/OCR honor the kill-switch; Home dual funnels (Decide via Travel Chat, Organize via Create/Import) with featured-trip Trip Copilot door; Trip Copilot evidence pack + packing proposals from saved facts only. Automated tests updated for DB v23 + evidence pack. Device smoke of remote `openai_compatible` still operator-gated (needs `AI_BASE_URL` / key in `server/.env`).
