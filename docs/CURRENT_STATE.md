...
The latest local git work is on `docs/expo-sqlite-rehearsal-7-15` through Elevation Program Waves 1–4, Wave 5 iOS config prep, Phase 6 local restore/credentials/EAS link, and two Android preview cloud builds (`ca98eafd-…`, then tab-bar `817e2c05-…`). Live Android `user_version` remains 17. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); this branch tracks `origin/docs/expo-sqlite-rehearsal-7-15` and may still be ahead of remote. EAS project `@velegris/TravelOS` is linked with a remote Android keystore and finished preview APKs. Preview Maps smoke on `817e2c05-…` passed 2026-09-06 (picker + Trip Map + World); Maps key Android-app SHA already registered. Play tracks remain open. iOS remains last.
