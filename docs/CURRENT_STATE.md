...
- Create Trip traveler timezone V1 is implemented on this branch: Create Trip can set or clear an explicit IANA timezone with `traveler` provenance. Replacing the map pin keeps a traveler timezone when the picker still has none. Coordinates are never used to guess a zone. Automated tests exist. No device rehearsal.
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, remaining Phase 5 visual matrix (Travelers/forms/CRUD), and iOS rebuild (last). The Android native rebuild and live v16/v17 upgrade are no longer parked.
