...
- Phase 6 local export + EAS profiles V1 is implemented on this branch: Profile → Export local backup writes `travelos.local-export.v1` JSON under `travelos/exports/` and shares it when `expo-sharing` is available. Export is read-only for SQLite and omits photo bytes. `eas.json` defines development/preview/production profiles; cloud EAS project linking is still required. Automated tests cover the export document contract.
