...
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, remaining Phase 5 visual matrix (booking save, stop/booking delete, trip delete confirm), and iOS rebuild (last). Owner toggle, unmapped Plan create, Companion Done, and Plan DONE badge on isolated Lisbon are no longer parked.
