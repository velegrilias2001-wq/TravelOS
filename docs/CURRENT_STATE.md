...
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, Android native rebuild for `expo-network` plus live v16/v17, Phase 5 visual matrix, and iOS rebuild (last). None of those were implemented in this pass.
...
Parked remaining work verification on 2026-09-03 re-ran `npx tsc --noEmit` and `npm test` (323 application tests passing). `git diff --check` was clean. Picker/Time Zone API, routes/ETAs, notifications, Discover rerank, OCR, Android native rebuild for `expo-network` plus live v16/v17, Phase 5 visual matrix, and iOS rebuild were recorded as deferred and were not implemented.
