...
- Historical Android Expo SQLite rehearsals verified upgrades through `user_version = 6`. This documentation pass did not re-open a device database, so live `PRAGMA user_version = 13` on an installed development build is **not** claimed. Node tests exist for versions 7–13. Equivalent iOS rehearsals are still outstanding.
...
- No device rehearsal of migration versions 7–13 on Expo SQLite.
