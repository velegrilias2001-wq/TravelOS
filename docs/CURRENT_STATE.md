...
The latest local git work is on `docs/expo-sqlite-rehearsal-7-15` through Elevation Program Waves 1–4, Wave 5 iOS config prep, Phase 6 local restore/credentials/EAS link, and two Android preview cloud builds (`ca98eafd-…`, then tab-bar `817e2c05-…`). Live Android `user_version` remains 17. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); this branch still has no upstream, so GitHub Actions has not run for it. EAS project `@velegris/TravelOS` is linked with a remote Android keystore and finished preview APKs. Play tracks and Maps package/SHA verification remain open. iOS remains last.
