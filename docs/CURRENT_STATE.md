...
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, and iOS rebuild (last). Pass 1–2 motion and Phase 5 visual matrix are no longer parked.
...
Parked remaining work verification on 2026-09-03 re-ran `npx tsc --noEmit` and `npm test` (323 application tests passing). `git diff --check` was clean. At that date, picker/Time Zone API, routes/ETAs, notifications, Discover rerank, OCR, remaining Phase 5 visual matrix, and iOS rebuild were deferred. By 2026-09-05 the Phase 5 visual matrix was closed on device; the other parked items and iOS remain deferred.
...
The immediate planned product-development sequence for **parked** items still needs a device session, a real provider, or iOS last: picker/Time Zone API (do not use the Android Maps key), real routes/ETAs, notifications, Discover rerank without installing BGE, confirmation-photo OCR, and iOS rebuild. Phase 5 visual/function matrix and polish V1 are closed. Phase 6 local export/restore, EAS profiles/project link, and credentials/versioning are implemented. Next operator steps are Play internal testing (optional), Maps package/SHA restriction verification with the EAS keystore fingerprint, then accounts/sync design — not iOS yet.
