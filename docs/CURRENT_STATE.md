...
- Pass 2 functional motion is implemented on this branch: Discover match cards stagger the first six items, wishlist bookmark pulse only when a save goes inactive→active, Companion/Map offline notices slide in when reachability is known, World filter list fades on Planned/Lived/All, and the splash glow is a static image (no multi-minute spin). A 2026-09-03 code audit confirmed those call sites. An Android Pixel 8 smoke on 2026-09-03 opened Home, Discover brief, World filters, and Nagawa Companion without crash.
