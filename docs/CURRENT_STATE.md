...
- Local-dev AI provider abstraction V1 is implemented on this branch: the `server/` package resolves `AI_PROVIDER` / `AI_CHAT_MODEL` / `AI_EMBEDDING_MODEL` (with `OLLAMA_*` aliases), exposes a single `createAiProvider()` for chat/embed, keeps a TravelOS copilot system prompt file, lists real vs `not_configured` tools at `/ai/tools`, and documents `openai_compatible` for a future dedicated Inference Endpoint without activating Hugging Face, Whisper, or a general chat UI. Feature flags: `AI_ENABLED`, `AI_SEMANTIC_SEARCH_ENABLED`, `AI_VISION_ENABLED`, `AI_RERANK_ENABLED`, `AI_VOICE_ENABLED=false`. See `server/.env.example`.
...
Current limitations include no production cloud AI provider, no on-device model, no BGE reranker install, no general chat/agent UI, and no silent conversion of advice into canonical stops. Local-dev AI boundary V1 keeps the client on loopback and ignores a non-local `EXPO_PUBLIC_TRAVELOS_AI_URL`. An `openai_compatible` provider path exists for a future dedicated Inference Endpoint but is not a shipping HF stack.
