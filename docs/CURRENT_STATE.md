...
- **Production brain docs (Wave E ε3)** are updated on this branch: `AI_PROVIDER=openai_compatible` remains the production path behind the same tool contract; privacy/retention/kill-switch (`AI_ENABLED=false`) documented in `server/.env.example` and Profile copilot copy. Native client still loopback-gates AI URLs.
