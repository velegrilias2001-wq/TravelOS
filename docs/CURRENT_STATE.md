...
- Routes, travel times, offline map behavior, clustering, and live navigation remain later work. Map day framing uses the assigned Day → Destination city plus that day’s mapped stops, or falls back to every saved coordinate. Directions hand off to the system maps app with the saved pin only.
...
- Android Expo SQLite now has a live `user_version = 15` rehearsal. Migrations 16 and 17 are in code and Node tests only. iOS still has no recorded migration-3, 7–15, 16, or 17 rehearsal.
...
- Map intelligence beyond day framing, in-app routes, travel times, and offline tiles. Map day framing + directions V1 frames the assigned city and that day’s mapped stops, and can hand a saved pin to Apple Maps or Google Maps.
