...
- Pass 1 functional motion is implemented on this branch: tab icon one-shot scale on select, `PressableScale` on Create Trip and booking save, Plan Done/Skipped `FadeIn`, shared `useReduceMotion`. No Lottie. Splash glow loop was not changed in this pass.
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, Pass 2 motion (Discover stagger, wishlist fill, offline banner), and iOS rebuild (last).
