...
- Current development branch: `feature/memory-day-stop-fk-v1`
- Branch point: `18f816a` — feat: require TripStop days to belong to the same trip
- Memories day/stop FK V1 is implemented on this branch: `memories.day_id` and `memories.stop_id` now have declared foreign keys (`ON DELETE SET NULL`). Dangling historical IDs are cleared; the Memory is kept. Travel Book memberships survive the table rebuild. Same-trip ownership remains in the existing v7 triggers. Migration version 15. Automated tests exist. No native device rehearsal.
...
- Historical Android Expo SQLite rehearsals verified upgrades through `user_version = 6`. This documentation pass did not re-open a device database, so live `PRAGMA user_version = 15` on an installed development build is **not** claimed. Node tests exist for versions 7–15. Equivalent iOS rehearsals are still outstanding.
...
Memories day/stop FK V1 verification on 2026-09-02 re-ran `npx tsc --noEmit` and `npm test` (249 application tests passing, including declared Memory foreign keys, day/stop unlink, Travel Book membership preservation, dangling-ID cleanup, and migration version 15). `git diff --check` was clean. No native rebuild was required. No device rehearsal was run for this integrity change.
...
- No device rehearsal of migration versions 7–15 on Expo SQLite.
...
The latest local git milestones on `feature/memory-day-stop-fk-v1` follow Stop-day same-trip V1 (`18f816a`). Memory day and stop IDs now have declared foreign keys. There is still no usable git remote for push.
