...
- Current development branch: `feature/import-calendar-zip-v1`
- Branch point: `4fb3091` — feat: add Import email-wrapped iCalendar V1
- Import calendar ZIP V1 is implemented on this branch: a chosen zip can contain `.ics` files, emails with a calendar part, or mixed non-calendar members. Nested zips are skipped. Extraction feeds the same review queue. PDF/Office/image extraction and AI parsing are still out. Automated tests exist. No native device rehearsal was completed for the zip extractor.
...
- Home, Trips, and Bookings can open `/import` to choose an `.ics` file, a zip of calendars, or paste iCalendar text. Extracted events enter a review queue and do not become bookings until accepted onto an existing trip.
...
Automated-test evidence: `tests/import-ics.test.cjs`, `tests/import-review.test.cjs`, `tests/import-review-migration.test.cjs`, `tests/import-calendar-file.test.cjs`, `tests/import-calendar-extract.test.cjs`, and `tests/import-calendar-zip.test.cjs`. Android Pixel 8 development-build evidence for paste/review/accept/delete, choosing a local `.ics` file, and extracting a `text/calendar` email on 2026-09-02 is recorded under Testing and release readiness. The zip extractor was not rehearsed on device in this pass.

Current limitations: PDF/Office/image extraction, itinerary-stop claims, and AI-assisted extraction are still out of scope. iOS has not been rebuilt with `expo-document-picker`. Accepted bookings still require the traveler to confirm reservation details in Bookings.
...
- Broader import formats. Native import now extracts iCalendar from paste, a chosen file, UTF-16 calendar bytes, an email `text/calendar` part, or a zip of those calendars. PDF/Office/image extraction and AI parsing remain out. Android Pixel 8 rehearsed choosing a local `.ics` file and an `.eml` with a calendar part; the zip extractor was not rehearsed on device in this pass. iOS was not rebuilt.
