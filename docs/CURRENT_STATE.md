...
The latest local git work is on `docs/expo-sqlite-rehearsal-7-15` through Phase 6 local restore, credentials/versioning, and EAS project link (`829355c`). Live Android `user_version` remains 17. Git remote `origin` exists (`https://github.com/veleg/TravelOS.git`); this branch still has no upstream, so GitHub Actions has not run for it. EAS project `@velegris/TravelOS` is linked. Store signing and the first cloud build are not yet run. iOS remains last.
