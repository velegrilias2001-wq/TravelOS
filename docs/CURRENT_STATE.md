...
- Pass 1 functional motion is implemented on this branch: tab icon one-shot scale on select, `PressableScale` on Create Trip and booking save, Plan Done/Skipped `FadeIn`, shared `useReduceMotion`. No Lottie.
- Pass 2 functional motion is implemented on this branch: Discover match cards stagger the first six items, wishlist bookmark pulse on save, Companion/Map offline notices slide in when reachability is known, World filter list fades on Planned/Lived/All, and the splash glow no longer runs a multi-minute spin. Automated tests pass (324). An Android Pixel 8 smoke on 2026-09-03 opened Home, Discover brief, World filters, and Nagawa Companion without crash.
