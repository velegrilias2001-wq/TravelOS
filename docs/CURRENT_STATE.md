...
- Light haptics V1 is implemented on this branch: `expo-haptics` 57.0.2 with `playSelectionHaptic` / `playLightImpact` that skip when Reduce Motion is on and fail closed if the native module is missing. Wired on tab select, Create Trip / booking save press, Companion Done/Skip pills, and Discover wishlist toggle. Feeling the taps on Pixel 8 still needs a development-build rebuild to autolink the module.
...
- Phase 5 polish V1 close is implemented on this branch: accessibility labels and ≥44pt targets on Home, World filters, Discover find chips, Memories, Travel Book, and Travel DNA; shared `EmptyState` / `InlineError`, `LocalImage`, `confirmDestructive`, and `MIN_TOUCH_TARGET`; locale-aware date/currency formatting via `locale-format` (English copy, device locale numbers/dates; RTL deferred); imagery contract in `docs/IMAGERY.md` (local traveler media only); written performance budgets below. Light haptics still need an Android development-build rebuild to feel on device. iOS layout/QA remains last.
- Phase 6 local export + EAS profiles V1 is implemented on this branch: Profile → Export local backup writes `travelos.local-export.v1` JSON under `travelos/exports/` and shares it when `expo-sharing` is available. Export is read-only for SQLite and omits photo bytes; restore is not implemented. `eas.json` defines development/preview/production profiles; cloud EAS project linking is still required. Automated tests cover the export document contract.
