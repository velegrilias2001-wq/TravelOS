...
- Booking sheet sticky footer and currency fix are implemented on this branch: the Add/Edit booking save button is now outside the ScrollView so it remains visible regardless of sheet content height. `KeyboardAvoidingView` wraps the sheet on iOS. `resolveBookingCurrencyCode()` omits currency when amount is empty, fixing the stale-EUR save failure. Delete booking rows expose `Delete {title}` accessibility labels. Automated tests cover the currency helper (324 passing). A Pixel 8 Phase 5 remainder script exists under `.test-build/phase5-remainder.ps1` (booking save/delete, stop delete, trip delete confirm with Cancel). Device rehearsal did not rerun: the emulator went offline after an earlier cold boot and did not recover online in this pass.
- End-to-end Pixel 8 pass (2026-09-03): Home (Nagawa HAPPENING NOW, 5 trips, 0 completed), Trips (all 5 listed with correct dates), Discover (all entry points visible), World (5 trips, 1 lived Lisbon, 6 planned, 7 places), Profile (Travel DNA, Saved ideas, 5 trips), Nagawa Companion (ON THE JOURNEY, Day 3, CITY NOT SET, Food stop with Done/Skip), Nagawa Plan/Map/Bookings/More (10 days, 6 moments, 1 map place, 1 booking, 2 memories), Lisbon Companion (ON THE JOURNEY, LISBON, Day 1, `17:05 in Europe/Lisbon` clock, RehearsalCoffee DONE with Undo), Lisbon Plan (1 day, 1 moment, DONE badge), Lisbon Bookings (0 — text-entry blocked), Lisbon More (1 traveler Alex, EUR, Planned). No crashes, no stale data, no regressions observed.
- Parked remaining work is recorded on this branch: picker/Time Zone API, real routes/ETAs, notifications, Discover rerank (no BGE install), confirmation-photo OCR, remaining Phase 5 visual matrix (stop/booking delete, trip delete confirm, booking text-entry on emulator), and iOS rebuild (last). Owner toggle, unmapped Plan create, Companion Done, Plan DONE badge, and booking sticky footer on isolated Lisbon are no longer parked.
- A 2026-09-03 Pixel 8 pass closed remaining import-extractor, wishlist, journey Create Trip, multi-destination, and Day → Destination device rehearsals on the installed `com.travelos.app` client. Import zip/PDF/Office/image files opened HIGH CONFIDENCE · PENDING reviews without writing bookings. Wishlist saved and then removed Slow days in Porto. Lisbon and Porto prefilling Create Trip with both cities; Trip Details then added Coimbra through the native location picker. Plan assigned Lisbon to Day 1. An isolated `Lisbon` trip (3 Sept 2026) remains on device with owner Alex and unmapped `RehearsalCoffee` marked Done. iOS was not exercised.
