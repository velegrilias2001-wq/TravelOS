...
Current limitations include no wishlist layer on World, no country statistics independent of saved destinations, and no backup/export of memory media. Saved Discover ideas live on their own list and are not World markers. Memories appear on lived World cards only through explicit day or stop IDs.
...
- World lived-history semantics beyond explicit done-stop labels plus ID-linked memories on lived cards. World archive V1 does not use trip status, destination titles, skipped stops, wishlist rows, or unlinked notes as visits. Media backup/export remains later.
