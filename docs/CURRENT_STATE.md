...
- Companion V2 lived progress and external live-data layers. Stop-boundary refresh V1 is implemented for canonical stop start/end times. Destinations may store an IANA timezone from catalogue, traveler, or a future provider field; the installed picker still does not return one. Two assigned cities claiming today still fall back instead of guessing.
