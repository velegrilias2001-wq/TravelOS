...
| Create Trip | Route-removal guard, complete local draft; successful creation explicitly releases guard before replace | TypeScript and navigation integration inspected | Native successful creation/previous step/picker return and discard cases still required |
...
| Inline FX | Route-removal guard and save-in-progress state; successful save resets baseline | TypeScript and runtime wiring inspected | Native validation, successful save and back/navigation cases |
