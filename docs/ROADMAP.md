...
- [x] Android Create Trip previous-step/date-picker cancellation, Keep editing, discard-without-write and successful creation; inline FX validation, retained-tab draft, discard-without-write, successful save/baseline reset. Cold relaunch preserved the synthetic trip/days/budget/FX through normal Home → Trips → Companion → More → Budget navigation. Exact fixture and temporary harness cleaned. System UI ANR/recovery and delayed transitions prevent treating this as a smooth cold-start/performance pass; no new release APK was built.
- [ ] Complete nested location/photo pickers, database-save failures, rapid-action cases, iOS sheet gestures and Android navigation variants. No process-death draft guarantee; modal protection is not full external route-unmount protection. See the editor matrix instead of inferring blanket coverage.
...
- [x] **Real routes / ETAs V1** — local-dev `POST /geo/directions` proxies Google Directions with a dedicated `GOOGLE_DIRECTIONS_API_KEY` (never Maps/Time Zone keys). Map draws provider polylines and shows walking duration/distance for consecutive mapped stops. Missing key, offline, or non-OK routes fail closed — no invented travel times. System Maps deep links remain for turn-by-turn. Automated tests exist.
...
Current Discover V1 includes Start with a place, Find me somewhere, Best time for a known catalogue destination, Ready-made journeys as catalogue ideas, Saved ideas persistence, a multi-pack grounded destination corpus, hybrid semantic extras from local retrieval when the AI server is available, optional listwise Discover rerank of already-retrieved identities when `AI_RERANK_ENABLED=true`, and opt-in grounded explanations of existing catalogue candidates. It does **not** include live provider catalogues or a BGE reranker install.
...
- [x] **Discover rerank V1 (listwise, no BGE)** — when `AI_RERANK_ENABLED=true`, Discover results may POST already-retrieved grounded identities to `/ai/discover-rerank` (qwen listwise). Invented identities fail closed; timeout/disabled/unavailable leave retrieve order unchanged. No BGE reranker install. Automated identity guards exist.
...
- [x] **Import image-embedded iCalendar V1** — JPEG, PNG, GIF, and WEBP files yield claims only when metadata or file bytes contain a `BEGIN:VCALENDAR` block, including compressed PNG zTXt and text split across chunks. Ticket photos without a calendar fail closed unless OCR V1 can read confirmation text. Automated tests exist. Android Pixel 8 rehearsal on 2026-09-03 chose `e2e-image-ferry.png` and reviewed `E2E Image Ferry` without writing a booking.
- [x] **Confirmation-photo OCR V1** — images without an embedded iCalendar may POST base64 to local-dev `POST /import/ocr-extract` (Cloud Vision via dedicated `GOOGLE_VISION_API_KEY`, gated by `AI_VISION_ENABLED`). Extracted text becomes import review claims only — never silent Booking/Trip writes. Unreadable photos and missing keys fail closed. Automated tests exist.
...
- [x] **iOS release prep V1 (config only)** — `ios.bundleIdentifier` `com.travelos.app`; `react-native-maps` receives `iosGoogleMapsApiKey` from the dedicated `GOOGLE_MAPS_IOS_API_KEY` (legacy `GOOGLE_MAPS_API_KEY` still accepted as a fallback); `expo-image-picker` config plugin sets Memory camera/photo usage strings. No iOS binary built on this Windows pass.
- Complete Android signing via EAS credentials, set `GOOGLE_MAPS_ANDROID_API_KEY` as an EAS secret for cloud builds, verify Maps key package/SHA restrictions, then preview/production Play tracks.
- [ ] **Operator step: issue split Maps keys.** The config now reads `GOOGLE_MAPS_ANDROID_API_KEY` and `GOOGLE_MAPS_IOS_API_KEY`. Create one Android-restricted key (Maps SDK for Android + Places API New, package `com.travelos.app` + signing SHA-1) and one iOS-restricted key (Maps SDK for iOS, bundle `com.travelos.app`), set both in `.env.local` and as EAS secrets, then retire the shared fallback. Until that is done the platform restrictions remain weaker than the engineering contract requires.
...
- [x] **Native Plan and Bookings smoke test** — both refactored screens driven end to end on `com.travelos.app.hardening` on 2026-09-17: stop create/edit/reorder, extracted stop editor and its close guard, booking create with payment state, extracted booking editor, cold-relaunch persistence and trip deletion. Location-picker return is still unverified because Places is blocked for that package.
- [x] **Explicit state when destination search saves nothing** — shared `requestLocationSelection` outcome plus an inline `LocationSearchNotice` with dismiss and retry, wired into Create Trip / Trip Details, the Plan stop editor and Accommodation. Verified natively against the 9011 condition. The copy names a possible cause without asserting one, because the picker cannot distinguish a cancellation from a provider failure.
...
- [x] **Corrected the `locale-format.ts` header** — it claimed copy stays English in V1, which the Greek migration made false. It now separates copy (the `src/i18n` catalogue) from formatting locale (device-resolved, `en-GB` fallback).
- [ ] Lower the `--max-warnings` ceiling as warnings are fixed. It sits at 48, the count when the gate was added; 20 of those are auto-fixable with `eslint --fix`.
- [ ] **Decide whether date and number formatting should follow the copy language.** `resolveDisplayLocale()` follows the device, so a device set to English shows Greek copy beside English-formatted dates such as "Sep 20, 2026" — visible in every Android screenshot from the migration. Pinning `el-GR` would make formatting consistent but would override a traveller's own device preference, which is why it is a decision rather than a fix.
...
- [x] Real routes / ETAs V1 (local-dev `POST /geo/directions` + Map polyline/ETA; fail closed; no invented times).
- [x] Trip notifications V1 (local stop-start reminders; opt-in; canonical timezone required).
- [x] Discover rerank V1 (optional qwen listwise on retrieved identities; `AI_RERANK_ENABLED`; no BGE install).
- [x] Confirmation-photo OCR V1 (Vision extract → import review claims only).
