...
- [ ] **Deferred — UX Refinement V1 Android visual/function matrix** — booking save, stop/booking delete, and trip delete confirmation still need an emulator pass. A 2026-09-03 Pixel 8 pass opened Companion/Plan/Map/Bookings/More on Nagawa, made Alex OWNER on isolated Lisbon, created an unmapped `RehearsalCoffee` place, opened the Add booking sheet, then previously cold-relaunched to Home with live `user_version` 17.
...
- Phase 5 visual/function matrix remainder: booking save, stop/booking delete, and trip delete confirmation. Owner toggle and an unmapped Plan create were rehearsed on Pixel 8 on 2026-09-03.
