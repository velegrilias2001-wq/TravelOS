...
- [x] **Multi-destination authoring V1** — Create Trip and Trip Details can add, reorder, and remove up to eight real map destinations. New destinations require a picker selection. A trip cannot drop to zero destinations once it has one. Removing a destination does not delete stops, bookings, or stays. Ready-made journeys can prefill extra catalogue cities. Day → Destination and timezone guessing remain out. Automated tests exist. No device rehearsal was run for this authoring flow.
- [ ] Adopt a stable provider identity when the chosen provider exposes one, and add Day → Destination semantics before claiming fully timezone-aware multi-destination presentation across Companion, Plan, Map, and Bookings.
